package top.benull.hitrecsys.movie.offline.spark.featureeng
...
object FeatureEngineering {
  ...
  val array2vec: UserDefinedFunction = udf { (a: Seq[Int], length: Int) =>
    Vectors.sparse(length, a.sortWith(_ < _).toArray, Array.fill[Double](a.length)(1.0))
  }
  ...
  def multiHotEncoderExample(samples: DataFrame): Unit = {
    // create new row for each element in the given array or map column
  ...
  val double2vec: UserDefinedFunction = udf { (value: Double) => Vectors.dense(value) }
